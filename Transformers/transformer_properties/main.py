...
def run_train_model(FLAGS):
    ...
    loss = transformer_model.evaluate(processed_dataset_test)
    
    logging("Test set - " + (" Loss = %0.3f" %(loss)),FLAGS)
...
def run_grid_search(FLAGS):	
    ...
    for params in itertools.product(drop_rate_set, d_model, n_layers, n_heads, 
                                    activation_func, ff_dim):
        ...
        encoder = transformer_model.train(processed_dataset_train,FLAGS, n_epochs, batch_size, lr_scheduler,
                                          lr_WarmUpSteps, min_delta, patience,
                                          optimizer_fn, p1, p2, p3, p4, p5,p6)
...
def run():    
    ...
    FLAGS.paths = {'chembl_data': 'data//ChEMBL_filtered.txt',#example_data
                   'usp7_data': 'data//smiles_dataset_3.csv'}
